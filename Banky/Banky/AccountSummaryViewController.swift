...
class AccountSummaryViewController: UIViewController  {
    ...
    // View Models
    var headerViewModel = AccountSummaryHeaderView.ViewModel(welcomeMessage: "Welcome 2", name: "", date: Date())
...
//                self.tableView.reloadData()
        ...
        group.notify(queue: .main) {
            ...
            self.isLoaded = true
            self.configureTableHeaderView(with: profile)
            self.configureTableHeaderView(with: self.accounts)
            self.tableView.reloadData()
        }
    }
    
    private func configureTableHeaderView(with profile: Profile) {
        let vm = AccountSummaryHeaderView.ViewModel(welcomeMessage: "Good Night", name: profile.firstName, date: Date())
